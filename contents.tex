...
are of interest in geometric modelling, see for instance \cite{Smith1989}.
...
\begin{align*}u_1\of{a,z}u_2\of{b,z}
...
\gn_{\pointset P}\of{B_1B_2}=\gn_{\pointset P_1}\of{B_1}\gn_{\pointset P_2}\of{B_2}\text.
\]
...
by a recurrence that splits the point set, yielding the Segner recurrence with inequality,
...
but one that does not decompose to the original alignments.\label{figWeirdAlignments}]{
\begin{tikzpicture}[scale=0.6]
\tikzstyle{dot}=[draw,shape=circle,fill=black,scale=0.5]
...
arbitrary crossing-free left-alignment $\gl$ of $\pointset L$ on $V$ and $\gr$ of $\pointset R$ on $V$.
...
since $\tuple{\gl,\gr}$ is overcounted; an example is shown in figure~\ref{figWeirdAlignments}.
\end{enumerate}
...
$\scone {j}$ for some $\pointset S\Subset\pointset L$ and some $j\in\set{1,\dotsc,\Cardinality{\pointset S}}$.
...
\begin{enumerate}
\item $\gk\of{\leftmost\pointset L}=\lcone j$ for some $j\in\set{1,\dotsc,\Cardinality{\pointset L}}$;
...
\end{scope}
\end{tikzpicture}
  \caption{The conifer maps the leftmost point of $\pointset L$ to $\lcone 3$.
...
for all crossing-free right-alignments $\gr$, $\tuple{\gl,\gr}$ is overcounted.
...
as observed at the end of the previous \sectionorchapterword, this will be an upper bound on the number of non-overcounted
crossing-free left-alignments, and by applying the same argument on the right, we will derive an upper bound on the number
of non-overcounted pairs of alignments---thus an upper bound on the number of crossing-free perfect matchings across a line.
...
$\pointset S^+$ below $\scone {i+1}$. We have $\Cardinality{\pointset S^-} = i$, and
$\Cardinality{\pointset S^+} = \Cardinality{\pointset S}-i-1$. Observe that any upper liminary points
of $\pointset S$ are liminary points of $\pointset S^-$, and any lower liminary points
of $\pointset S$ are liminary points of $\pointset S^+$.
...
$\gD_2$ satisfying $\gD_1+\gD_2=\gD-1$ (since the leftmost point is liminary, but neither upper nor lower liminary).
Given the numbers of upper and lower liminary points, we take the sum over the choices of
$\gk\of{\leftmost\pointset L}=\lcone {i+1}$, excluding values of $i$ which correspond to vacuous cones by the
...
The base of the exponential is $4$ for $\gX=0$, and it tends towards $1$ as $\gX$ tends towards $1$.
...
$c$ left-points forming $\pointset C$, and $d$ right-points forming $\pointset D$.
...
The matching is uniquely determined by $\ga$, $\gd$, the choice of
the subsets $\pointset S\Subset\pointset A$ and $\pointset T \Subset D$,
and by $\gb$ and $\gg$. This gives the following bound for the number of these matchings:
\begin{equation}
...
$p\in\pointset B_i$,
\[\gb\of p = \gb_i\of p + \sum{j=1}[i-1] \Cardinality{\pointset B_j}\text.\]
...
with $\spc\of{a,b}=0$ for $b<0$; this is consistent with the original definition, since it yields a sum over
...
by chaining
\begin{enumerate}
\item a prefix with $i$ opening brackets containing the first $\gD_1$
unmatched opening brackets;
\item an opening bracket, which will be unmatched;
\item a prefix with $k-i-1$ opening brackets containing the last $\gD_2$
unmatched opening brackets.
\end{enumerate}
Indeed, such a decomposition of is unique, since $i$ is the number of opening brackets
before the $\pa{\gD+1}$th unmatched opening bracket.
...
\]
In particular, we have the trivial maximum (over equal values)\[
...
its left side in a known vertical order, we choose which ones will cross the strip. This determines the sizes of
...
When we finally reach the points $\pointset D$ corresponding to $\rangle^d$, we pick a crossing-free
right-alignment of them.
...
\]
where in the first sum $i$ corresponds to the choice of the left cone $\lcone {i+1}$,
and in the second sum $j$ corresponds to the choice of the right cone $\rcone {j+1}$.
