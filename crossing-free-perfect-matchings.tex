...
from crossing-free left-alignments of $L$ on any line to left conings. 
...
and we will call vacuous a left coning that has vacuous cones; thus $\gl$ is overcounted
if $\conification_\gl$ is vacuous.

We will now use convexity considerations to give an upper bound on the number of non-vacuous left conings.
...
$\gd$ $R$-liminary points of a set of $k$ points, we can give the following upper bound on the number of
non-vacuous left conings
\[\max_{\gd_1+\gd_2=\gd-1}\sum{i=\gd_1}[k-\gd_2]\varpi_{i}\varpi_{k-i-1}\text.\]
Moreover, for any non-vacuous left $L$-cone $\gk$, any upper $R$-liminary points of $L$ are $R$-liminary points of the
subset of $L$ above $\gk$, and any lower $R$-liminary points of $L$ are $R$-liminary points of the
subset of $L$ below $\gk$, so that we can get the following recurrence
\[\varpi_{k,\gd}\DefineAs\max_{\gd_1+\gd_2=\gd-1}\sum{i=\gd_1}[k-\gd_2]\varpi_{i,\gd_1}\varpi_{k-i-1,\gd_2}\]
where $\varpi_{k,\gd}$ is an upper bound on the number of crossing-free non-vacuous left conings of $k$ points
