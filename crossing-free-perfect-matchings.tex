...
\[\spc\of{k, l} = \sum{j=k-l}[k]
C_{k-j} \quad
\sum{s\in\binom{[j-1]}{k-l - 1}} \qquad
...
\]
Rewriting this as as sum over $i\DefineAs k-j$,
\[\spc\of{k, l} = \sum{i=0}[l]
C_{i}
\spc\of{k-i-1,l-i}\text.
\]\marginfig[The $l+1$ summands in the $\spc$ recurrence.]{TODO}
We can now turn this recurrence into a simpler recurrence, which we prove by recurrence.
...
Moreover, note that for $k - 1 = l$, we have
\begin{align*}
\spc\of{k, l}
&= \sum{i=0}[l]C_{i}\spc\of{k-i-1,l-i} = \sum{i=0}[l]C_{i}\spc\of{l-i,l-i}\\
&= \sum{i=0}[l]C_{i}C_{l-i} = C_{l+1}
\end{align*}
