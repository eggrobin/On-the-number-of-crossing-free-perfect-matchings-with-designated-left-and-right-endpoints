...
In the study of plane embedding of planar graphs, embeddings with non-crossing straight-line edges (called crossing-free
geometric graphs) are of particular
interest.
For instance, it is known [CITE the Dutch paper] that any optimal solution to the travelling salesman problem in the
Euclidean plane.

In in 1936, K.~Wagner proved\footnote{This result was independently proved by I.~Fáry in 1948 [CITE], and by
...
For $n$-cycles, the question was introduced by M.~Newborn and W.~O.~J.~Moser in 1976 [CITE], and a super-exponential
upper bound was given.
In 1980, M.~Ajtai, V.~Chvátal, M.~Newborn, and E.~Szemerédi proved that there are only exponentially many
crossing-free geometric graphs on a given point set.
...
M.~Noy, and J.~Tejel; upper bounds have been harder to come by, with the best one yet being (asymptotically,
and disregarding polynomial factors) $10.05^n$, given by M.~Sharir and E.~Welzl in 2005 [CITE].

[TODO blurb about history and applications of crossing-free graphs; for instance
that untangling paper has the fact that an optimal solution to the traveling salesman
is crossing-free]
...
\draw[dashed] (p6) -- (b6);
\end{tikzpicture}}
Construct a bracket expression $\gb\of\gm$ of length $n$ as follows: order $P$ from left to right;
...
$\gb^{-1}\of{B}\Intersection \CFPM_P$ for bracket expressions $B$ of length $n$\idest the
...
The following proof is due to A.~J.~Ruiz-Vargas and E.~Welzl [TODO cite paper to appear, is there a preprint?].
...
eventually\footnote{The entangling procedure itself predates this proof, and was shown to terminate in
$\BigO\of{n^3}$ steps by J.~van~Leeuwen and A.~A.~Schoone in 1980 [TODO citation].}
find a
...
by a recurrence that split the point set, rather than via well-formed bracket expressions [TODO
citation].
\begin{corollary}
...
The following result, and its proof, were given by Micha Sharir and Emo Welzl in 2005 [TODO cite].
\NoEndMark
...
$\gm = \gl \gr^{-1}$\idest that there can be at most $C_k^2$ perfect matchings across a line.
