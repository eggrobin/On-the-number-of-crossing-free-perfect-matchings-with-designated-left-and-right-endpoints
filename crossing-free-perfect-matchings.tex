...
(\ref{SharirWelzlOvercounting}), but more importantly, it tells us that $\gn_{\pointset P}\of{\langle^k\rangle^k}$
...
and examine it in more detail, with some fresh definitions.
...
realizing $\gl$, the segment $\gs_{\leftmost L}$ of the leftmost point then has to be contained in left
...
to the left side of $V$ realize the left-alignment in $\decomposition\of\gm$. Thus if $\tuple{\gl, \gr}$ is in the image of
$\decomposition$, $\leftmost L$ has to be matched to a right-point in left $\pointset L$-cone number $\gl\of{\leftmost L}$, see
...
\item $\gk$ restricted to the points above the interior of $\gk\of{\leftmost L}$ is a left conifer of these points;
\item $\gk$ restricted to the points below the interior of $\gk\of{\leftmost L}$ is a left conifer of these points.
\end{enumerate}
...
of the points above and below any edge realizing $\gl$\idest above and below cone number $\gl\of{\leftmost L}$.
