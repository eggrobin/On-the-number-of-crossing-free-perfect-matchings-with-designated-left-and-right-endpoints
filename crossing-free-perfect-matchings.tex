...
In this section and the following one, crossing-free left- and right-alignments will implicitly be on $V$,
and unless otherwise mentioned, crossing-free left-alignments will be of $\pointset L$, and crossing-free
right-alignments will be of $\pointset R$.
...
\]
so that $\Cardinality{\LC {\pointset L}} = \CatalanNumber {\Cardinality{\pointset L}}$.
...
We will call $\leftmost\pointset L$ the leftmost point of $\pointset L$; note that it is an $\pointset R$-liminary point
of $\pointset L$.

Note that in polar order around $\leftmost\pointset L$, the $\pointset R$-liminary points of $\pointset L$ other than
...
If there are $\gD^-$ upper $\pointset R$-liminary points of $\pointset L$, then the first $\gD^-$ left
$\pointset L$-cones are vacuous.
Similarly, if there are $\gD^+$ lower $\pointset R$-liminary points of $\pointset L$, then the last
...
where $\gcp_{k,\gD}$ is an upper bound on the number of crossing-free non-vacuous left conifers on $k$ points
