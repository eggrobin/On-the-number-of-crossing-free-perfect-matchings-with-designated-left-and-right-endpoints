...
As a result,
the strip $\gS$ between the vertical lines $V$ and $W$ is partitioned in trapezoidal cells
$Z_1,\dotsc,Z_{a-b+1}$ by the long edges, see figure~\ref{figTrapezoids}.

We will consider the crossing-free left-alignment $\gb$; the same argument will hold for $\gg$.
Since $\pointset B$ lies within the strip $\gS$, it is partitioned in subsets
$\pointset B_1\Subset Z_1,\dotsc,\pointset B_{a-b+1}\Subset Z_{a-b+1}$.
...
$\gb$ is composed of the crossing-free right-alignments $\gb_i$, formally,
\[\Restrict{\gb}{\pointset B_i}\of p = \gb\of p - \sum{j=1}[i-1] \Cardinality{\pointset B_i}\text.\]

Let us look at the edges crossing $V$, numbered from top to bottom: $k-l$ of those are
long edges; they are the edges emanating from $\pointset S$. 
Let $S = \ga\of{\pointset S} \Subset [k]$ be their numbers. in between two long edges,
above the first
