...
\Cardinality{\CFPMAL_P \arrow{r}{\gw}} \leq
\Cardinality{\CFLA_{L,V}\Cartesian\CFRA_{R,V} \arrow{ld}{c}}\text, \label{SharirWelzlOvercounting}
\end{equation}
...
the image of $\gw$, since they are responsible for that excess.
Evidently, if $c\of{\gl,\gr}$ is crossing, then $\tuple{\gl,\gr}$ is not in the image of $\gw$,
since the diagram commutes. On the other hand, figure~\ref{figWeirdAligments} shows a pair of
