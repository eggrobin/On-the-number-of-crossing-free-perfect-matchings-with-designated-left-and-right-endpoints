...
Then, removing that triangle from any triangulation yields triangulations
of point sets of size $i$ (containing points $0$ through $i-1$) and $n-i-1$ (containing points $i+1$ through
$n-2$), so $C_n \leq \sum {i=0}[n-1] C_i C_{n-i-1}$.
Moreover, any triangulations of these two point sets, together with the triangle $\set{n,n-1,i}$
and the edges $\set{n+1,0}$, $\set{n, n-1}$, $\set{i-1, i}$, and $\set{i, i+1}$, form a triangulation of the
whole point set, proving equality.
\end{proof}
\end{theorem}
...
counts the number of parenthesizings of $n+1$ factors.} by E.~Catalan in 1838 \cite{Catalan1838}, that the
