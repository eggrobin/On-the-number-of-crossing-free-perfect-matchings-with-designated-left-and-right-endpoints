...
\node[dot,label=above:$2i+1$] at (4,4) (p2iplus1) {};
\node[dot,label=right:$2i+2$] at (5.5,3) (p2iplus2) {};
\node[dot,label=right:$2n-2$] at (5,1) (p2nminus2) {};
\node[dot,label=below:$2n-1$] at (3,0) (p2nminus1) {};
\draw (p0) -- (p2iplus1);
\end{tikzpicture}}
...
The bound given in the previous section for the number of matchings across a line is not optimal;
...
An overcounted pair of alignments $\composition\of{\gl,\gr}$ falls into one of two families:
\begin{enumerate}
\item
$\composition\of{\gl,\gr}$ can be a crossing perfect matching, as in figure~\ref{figCrossingPMAL};
This implies that $\tuple{\gl,\gr}$ is not in the image of $\decomposition$, since the diagram commutes;
...
We would therefore like to give a bound on the number of non-overcounted pairs of alignments; in order to do that,
...
First, for a point set $\pointset L$ in general position with respect to the horizontal,
note that we can partition the half-plane to the right of $\leftmost\pointset L$ in
$\Cardinality{\pointset L}$ regions,
separated by rays emanating from $\leftmost\pointset L$ and going through other points of
$\pointset L$, see figure~\ref{figCones}.
We will call those regions \emph{left $\pointset L$-cones}, and call them, from top to bottom, $\lcone 1$ to
$\lcone {\Cardinality{\pointset L}}$.
Note that there are $i$ points of $\pointset L$ above the interior of $\lcone i$,
and $\Cardinality {\pointset L} - i - 1$ points below.

Observe that the same separation of the point set held for realizing segments of crossing-free left-alignments,
see for instance figure~\ref{figCrossingFreeAlignment}.
As a consequence, given a vertical line $V$ to the right of $\pointset L$, and a crossing-free left-alignment
$\gl$ of $\pointset L$ on $V$, for any set of segments $A=\setst{\gs_p}{p\in L}$
realizing $\gl$, the segment $\gs_{\leftmost L}$ of the leftmost point has to be contained in left
$\pointset L$-cone number $\gl\of{\leftmost L}$.
...
To extend this reasoning beyond the leftmost point of $\pointset L$, we define \emph{left conifers on $\pointset L$}.
A left conifer $\gk$ on a point set $\pointset L$ is a map from points in $\pointset L$ to left
$\pointset S$-cones for subsets $\pointset S$ of $\pointset L$,
satisfying the following properties:
\begin{enumerate}
\item $\gk\of{\leftmost L}$ is a left $\pointset L$-cone;
\item $\gk$ restricted to the points above the interior of $\gk\of{\leftmost L}$ is a left conifer on these points;
\item $\gk$ restricted to the points below the interior of $\gk\of{\leftmost L}$ is a left conifer on these points.
\end{enumerate}
...
\end{tikzpicture}
  ...
  as that point.}
\end{subfigure}
\caption{The construction of a left conifer a set $\pointset L$ of five points.}
...
is determined by the value of $\gl\of{\leftmost L}$ and by
crossing-free left alignments $\gl^-$ and $\gl^+$ on $V$
of subsets $\pointset S^-$ and $\pointset S^+$ above and below any edge
realizing $\gl$\idest subsets above and below $\lcone{\gl\of{\leftmost L}}$.
Formally, with
\begin{align*}
\pointset S^- &\DefineAs \gl^{-1}\of{[\gl\of{\leftmost\pointset L}]}\text,\\
\pointset S^+ &\DefineAs \gl^{-1}\of{[\Cardinality{\pointset L}]\setminus[\gl\of{\leftmost\pointset L}]}\text,\\
\gl^-\of{p} &\DefineAs \gl\of{p} \text{ for $p\in\pointset S^-$,}\\
\gl^+\of{p} &\DefineAs \gl\of{p}-\gl\of{\leftmost\pointset L} \text{ for $p\in\pointset S^+$,}
\end{align*}
$\gl^\pm$ is a crossing-free left-alignment of $\pointset S^\pm$ on $V$.
...
\FunctionDefinition {\conification} {\CFLA_{\pointset L, V}} {\LC L} {\gl} {\conification_\gl}
\]
...
$\tuple{\gl,\gr}$ is overcounted. We will call a left cone \emph{$\pointset R$-vacuous} if it does not contain a point
from $\pointset R$. 
Moreover, we will write that a conifer $\gk$ is \emph{$\pointset R$-vacuous} if for some $p\in\pointset L$,
...
are $\pointset R$-vacuous.\label{figLiminaryPoints}]{
\begin{tikzpicture}[scale=0.75]
\tikzstyle{dot}=[draw,shape=circle,fill=black,scale=0.5]
...
We will call $\leftmost L$ the leftmost point of $\pointset L$; note that it is an $\pointset R$-liminary point
of $\pointset L$.

Note that in polar order around $\leftmost L$, the $\pointset R$-liminary points of $\pointset L$ other than
$\leftmost L$ are separated by
