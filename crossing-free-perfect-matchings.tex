...
maximal crossing-free geometric graphs. Indeed, finding a bound for them yields a bound
...
l \of n \leq g \of P \leq u \of n \text,\]
...
\begin{corollary}
Let $P$ be a point set of even size $n$ in general position. There are at least $C_{\frac n 2}$ distinct
...
A bijection $\gl$ from a set of $\gL$ to $k$ is a \emph{crossing-free left-alignment of $\gL$ on $V$} if
there exists a set $A=\setst{a_p}{p\in\gL}$ of $k$ line segments such that
the following hold:
\begin{enumerate}
...
For a point $p\in\gL$, we will call $a_p$ \emph{the segment of $p$}.
...
$\gd_2$ left $L$-cones are vacuous. Moreover, for any non-vacuous left $L$-cone $\mathscr{k}$, any upper $R$-liminary points
of $L$ are $R$-liminary points of the
subset of $L$ above $\mathscr{k}$, and any lower $R$-liminary points of $L$ are $R$-liminary points of the
subset of $L$ below $\mathscr{k}$, so that we get the recurrence
