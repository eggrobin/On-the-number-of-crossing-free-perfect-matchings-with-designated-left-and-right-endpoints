...
We now solve the recurrence $\gcp$ defined in section~\ref{sectionHighlyConvex}. Recall that $\gcp$ is defined by
\begin{align*}
\gcp_{k,\gD}&\DefineAs\max_{\gD^-+\gD^+=\gD-1}\sum{i=\gD^-}[k-\gD^+-1]\gcp_{i,\gD^-}\gcp_{k-i-1,\gD^+}\text,\\
...
\]
In particular,\[
\gw_{k,\gD}=\max_{\gD^-+\gD^+=\gD-1}\sum{i=\gD^-}\sum{i=\gD^-}[k-\gD^+-1]\gw_{i,\gD^-}\gw_{k-i-1,\gD^+}\text.
\]
