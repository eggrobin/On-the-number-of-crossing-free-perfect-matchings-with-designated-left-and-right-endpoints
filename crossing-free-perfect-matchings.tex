...
has to fulfill in order not to be overcounted.
...
Further, note that we can partition the half-plane to the right of $\leftmost\of L$ in $\Cardinality L$ regions,
...
realizing $\gl$, the segment $a_{\leftmost\of L}$ of the leftmost point has to be contained in left
...
interior of that cone, see figure~\ref{figRecursiveConing}. This is exactly the Sharir--Welzl bound,
...
Call $\cone_\gl p$ the left $S$-cone, $S\Subset L$, associated to point $p\in L$ in that fashion.
