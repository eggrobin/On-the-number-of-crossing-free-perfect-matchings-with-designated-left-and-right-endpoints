...
construction $\decomposition$, thus $\decomposition\of{\gm}=\tuple{\gl, \gr}$. Moreover, let us call $\CFLA_{\pointset L, V}$ the
...
let $\composition\of{\gl,\gr}$ be the (possibly crossing) perfect matching $\gr^{-1}\Compose\gl$.
...
$\decomposition$ must be injective, and so that\begin{equation}
...
the image of $\decomposition$, since they are responsible for that excess; we will call them \emph{overcounted}.
Overcounted pairs of crossing-free alignments fall into one of two categories;
\begin{enumerate}
\item
evidently, if $\composition\of{\gl,\gr}$ is crossing, then $\tuple{\gl,\gr}$ is not in the image of $\decomposition$,
...
$\gm = \composition\of{\gl,\gr}$, but which is not equal to $\decomposition\of\gm$.
\end{enumerate}
...
First, for a point set $\pointset L$, note that we can partition the half-plane to the right of $\leftmost L$ in
$\Cardinality L$ regions,
separated by rays emanating from $\leftmost L$ and going through other points of $\pointset L$. We will call those regions
