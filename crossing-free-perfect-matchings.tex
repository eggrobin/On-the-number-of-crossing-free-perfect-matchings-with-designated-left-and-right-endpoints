...
We will call $L_1$ the leftmost point of $L$; note that it is an $R$-liminary point of $L$.

Note that in polar order around $L_1$, the $R$-liminary points of $L$ other than $L_1$ are separated by
$R$, that is, in polar order, one subset of the $R$-liminary points of $L$ appears first, followed by
all points of $R$, followed by the rest of the $R$-liminary points of $L$ (except $L_1$).
...
$\gw$, $L_1$ has to be matched to a right-point in cone number $\gl\of{L_1}$, see figure~\ref{figConesNeedPoints}.

Moreover, since a crossing-free left-alignment $\gl$ is defined by $\gl\of{L_1}$ and crossing-free left alignments
of the points above and below any edge realizing $\gl$\idest above and below cone number $\gl\of{L_1}$,
we can recursively apply this argument, with each left-point having to be matched to a right-point a cone defined on
a subset of the left-points, see figure~\ref{figRecursiveCones}.
