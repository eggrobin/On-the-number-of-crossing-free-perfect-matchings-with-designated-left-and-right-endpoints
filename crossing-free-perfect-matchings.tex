...
of the leftmost point $\leftmost \pointset S$ is partitioned in
...
The \emph{extreme points of $\pointset P$} are the points of $\pointset P$ that
are in the boundary of the convex hull of $\pointset P$.
The \emph{liminary points of $\pointset L$} are the points of $\pointset L$ which are extreme points
of $\pointset P = \pointset L\Union \pointset R$,
see figure~\ref{figLiminaryPoints}.
Note that $\leftmost\pointset L$ is a liminary point of $\pointset L$.
...
of which at least $\gD$ are liminary.
Moreover, we can start that recurrence with
\begin{align*}
\gcp_{k,0}&\DefineAs\gcp_{k,1}\text,\\
\gcp_{k,k}&\DefineAs1\text,
\end{align*}
since there is at least one liminary point of $\pointset L$ (the leftmost one), and if all points are
liminary, only one cone is non-vacuous.
...
\gcp_{k,\gD}=TODOTODOTODO\text,
\]
and thus that\[
\gn_{\pointset P}\of{\langle^k\rangle^k}\leq TODOTODOTODO\text.
\]
